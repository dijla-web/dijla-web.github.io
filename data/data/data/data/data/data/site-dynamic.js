/*
  site-dynamic.js
  ----------------
  يقرأ هذا السكربت ملف data/news.json فقط، ويضيف عناصره كأخبار/تحديثات جديدة
  فوق التصميم الحالي — لا يلمس أي عنصر آخر بالموقع ولا يعدّل أي نص موجود مسبقاً.
  إذا تعذّرت القراءة أو كان الملف فارغاً، لا يحدث أي تغيير على الصفحة إطلاقاً.
*/
(function () {
  'use strict';
  ...
  function renderHomeNews(items) {
    ...
    // الأحدث أولاً — ندرجها بالأعلى، فوق الأخبار الخمس الأصلية الثابتة بالموقع
  ...
  // يُستدعى من onclick المُولَّد أعلاه
  ...
  function renderPageUpdates(homeNews) {
    ...
    var byPage = {}; // pageContainerId -> array of items (فرز صارم حسب فئة كل خبر فقط)

    homeNews.forEach(function (it) {
      var containerId = CATEGORY_TO_PAGE_CONTAINER[it.category];
      if (!containerId) return; // فئة غير معروفة/غير مصنّفة → لا تُعرض بأي صفحة مخصصة (يبقى فقط بقائمة الرئيسية)
  ...
  function init() {
    fetch('data/news.json', { cache: 'no-store' })
      ...
      .catch(function () {
        /* لا داعي لأي إجراء — الموقع يبقى يعمل بشكل طبيعي إذا تعذّرت القراءة */
      });
  }
